...
public class SBPotions {
	public static final Supplier<Potion> PARALYSIS_POTION = register("paralysis_potion", () -> new Potion("paralysis", new MobEffectInstance(SBEffects.PARALYSIS.get().value(), 900, 0)));
	...
	private static <T extends Potion> Supplier<T> register(String name, Supplier<T> supplier) {
		return SlayersBeasts.PLATFORM.register(BuiltInRegistries.POTION, name, supplier);
	}
